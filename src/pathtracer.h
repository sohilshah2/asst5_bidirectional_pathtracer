...
namespace CMU462 {
  ...
  struct LightPath {
    int length;
    std::vector<Spectrum> illum;
    std::vector<float> pdf;
    std::vector<Intersection> isect;
    std::vector<Ray> ray;
  };
  ...
  private:
     ...
     */
    Spectrum trace_camera_ray(const Ray &r, const struct LightPath& p, int bounce, bool includeLe);
    void trace_light_ray(SceneLight *light, struct LightPath& p, int bounce);
